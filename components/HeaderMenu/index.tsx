...
const HeaderMenu = () => {
  const headerMenu = useSelector(
    (state: RootState) => state.common.topCategories
  );
    ...
    <nav className="d-xs-none">
      <div className="inner-content">
        ...
        <ul className="unexpanded">
          ...
          {headerMenu &&
            headerMenu.data &&
            headerMenu.data.map((menu: any) => (
              <li className="nav-item dropdown list-item-group">
                <Link href={`/${menu.category_slug}`}>
                  <a
                    ...
                    <div className="row">
                      {menu.sub_category.length > 0 && (
                        <div className="col-3 mt-3 mb-3">
                          <div>
                            {chunk1(menu.sub_category, 0, 9).map(data => (
                              <Link
                      ...
                      {menu.sub_category.length > 10 && (
                        <div className="col-3 mt-3 mb-3">
                          <div>
                            {chunk1(menu.sub_category, 10, 19).map(data => (
                              <Link
                                href={`/${menu.category_slug}/${
                                  data.category_slug
                                }`}
                      ...
                      {menu.sub_category.length > 20 && (
                        <div className="col-3 mt-3 mb-3">
                          <div>
                            {chunk1(menu.sub_category, 20, 29).map(data => (
                              <Link
                                href={`/${menu.category_slug}/${
                                  data.category_slug
                                }`}
                      ...
                      {menu.sub_category.length > 30 && (
                        <div className="col-3 mt-3 mb-3">
                          <div>
                            {chunk1(menu.sub_category, 30, 39).map(data => (
                              <Link
                                href={`/${menu.category_slug}/${
                                  data.category_slug
                                }`}
          ...
          <li className="nav-item dropdown list-item-group">
            <a className="nav-link">
              <span>
                Saved <i className="fa fa-heart-o" />
              </span>
            </a>
          </li>
        </ul>
